<?php session_start();
    ...
    $new_value = 1;
    $reactivate_wallet = $object->update_with_one_param($table,$param,$value,$new_value_param,$new_value);
    $reactivate_wallet_decode = json_decode($reactivate_wallet, true);
    $get_user = $object->get_one_row_from_one_table('users_tbl','unique_id',$unique_id);
    $username = $get_user['surname'].' '.$get_user['other_names'];
    if($reactivate_wallet_decode['status'] == 0){
    	echo "error";
    }else{
    	echo "success";
