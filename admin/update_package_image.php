...
if(isset($_POST['cmd_update'])){
      ...
      $package_id = $_POST['package_name_img'];


     $img_create = $object->update_package_image($package_id,$file_name, $size, $tmpName, $type);
     $img_dec = json_decode($img_create,true);

      if($img_dec['status'] == 1){
     $msg = "<div style='color:green;'>Image Update was successful<a href='view_packages'>Return to Packages</a></div>";
        ...
        <div class="col">
          <div class="card shadow">
            <div class="card-header border-0">
              <h3 class="mb-0">Update Package Image</h3>
              <?php if(!empty($msg)){
                echo $msg;
              }?>
            </div>
              ...
              <div class="row">
                    ...
                    <div class="col-lg-8">
                      <form  enctype="multipart/form-data" action="" method="post" id="cat_img_form"> 
                      <!-- <div class="form-group"> -->
                      
                        <label class="form-control-label" for="input-first-name">Select Package</label>
                        <!-- <input type="text" name="cat_name" id="cat_name"  class="form-control"> -->
                        <select class="form-control" name="package_name_img" id="package_name_img">
                            <option value="">Please select a package</option>
                            <?php  foreach($get_rows as $pack){?>
                             <option value="<?php echo $pack['unique_id']; ?>"><?php echo $pack['package_name']; ?></option>
                            <?php } ?>
                        </select>
                        <br>
